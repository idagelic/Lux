...
class Machic_Vendor_Grid_Widget extends Widget_Base {

    public function get_name() {
        return 'machic-vendor-grid';
    }
	...
	protected function register_controls() {
        ...
        $this->add_control( 'display_items',
            [
                'label' => esc_html__( 'Display Item count', 'machic-core' ),
	...
	protected function render() {
		...
		if ($vendors && is_array($vendors)) {
			...
			foreach ($vendors as $vendor_id) {
				...
				$vendor = get_mvx_vendor($vendor_id->id);
				...
				if($i <= $settings['display_items'] && !get_user_meta($vendor->id, '_vendor_turn_off', true)){
					...
					echo '<p>'.machic_sanitize_data($vendor->description).'</p>';
					echo '</div>';
					echo '</div>';
					echo '</div>';
					echo '</div>';
