...
class Machic_Category_Banner3_Widget extends Widget_Base {
    use Machic_Helper;
	
    public function get_name() {
        return 'machic-category-banner3';
    }
    public function get_title() {
        return 'Category Banner 3 (K)';
    }
	...
	protected function render() {
		...
		$output .= '<div class="site-module products-category-module style-4">';
		...
		foreach ( $terms as $term ) {
			...
			$output .= '<h4 class="entry-title">'.esc_html($term->name).'</h4>';
			if($term_children){
				$count = 0;
		...
		$output .= '<div class="banner dark align-center">';
		...
		$output .= '<a href="'.esc_url($settings['banner_btn_link']['url']).'" '.esc_attr($bannertarget.$bannernofollow).' class="btn small rounded link-color">'.esc_html($settings['banner_btn_title']).'</a>';
		...
		$output .= '<a href="'.esc_url($settings['banner_btn_link']['url']).'" '.esc_attr($bannertarget.$bannernofollow).' class="overlay-link"></a>';
		$output .= '</div><!-- banner -->';
