...
if(get_theme_mod('machic_paginate_type') == 'loadmore' || machic_ft() == 'load-more'){
	function machic_load_more_button(){
		echo '<nav class="woocommerce-pagination klb-load-more">
                <ul class="page-numbers">
					<div class="button light">'.esc_html__('Load More','machic-core').'</div>
				</ul>
			  </nav>';
	}

	remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
	add_action( 'woocommerce_after_shop_loop', 'machic_load_more_button', 10 );
}
...
if(get_theme_mod('machic_paginate_type') == 'infinite' || machic_ft() == 'infinite'){
	remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
}
...
if(get_theme_mod('machic_ajax_on_shop',0) == '1'){
	add_action('woocommerce_before_main_content','machic_loadmore_parameters',40);
	function machic_loadmore_parameters() {
	...
	<script type="text/javascript">
		var loadmore = {
			...
			"no_more_products":"<?php echo esc_html_e('No More Products','machic-core'); ?>",
...
## Quick View Scripts
*************************************************/
if(get_theme_mod('machic_paginate_type') == 'loadmore' || get_theme_mod('machic_paginate_type') == 'infinite' || machic_ft() == 'load-more' || machic_ft() == 'infinite'){
	if(get_theme_mod('machic_ajax_on_shop',0) == '1'){
		function machic_load_more_scripts() {
			if(is_shop() || is_tax()){
				if(get_theme_mod('machic_paginate_type') == 'infinite' || machic_ft() == 'infinite'){
					wp_enqueue_script( 'machic-load-more',  plugins_url( 'js/infinite-scroll.js', __FILE__ ), true );
				} elseif(get_theme_mod('machic_paginate_type') == 'loadmore' || machic_ft() == 'load-more') {
					wp_enqueue_script( 'machic-load-more',  plugins_url( 'js/load_more.js', __FILE__ ), true );
				}
			}
		}
		...
		function machic_load_more_scripts() {
			if(is_shop() || is_tax()){
				...
				wp_localize_script( 'machic-load-more', 'loadmore', array(
					...
					'no_more_products' => esc_html__('No More Products','machic-core'),
	...
	add_action( 'wp_enqueue_scripts', 'machic_load_more_scripts' );
}
...
add_action( 'wp_ajax_nopriv_load_more', 'machic_load_more_callback' );
add_action( 'wp_ajax_load_more', 'machic_load_more_callback' );
function machic_load_more_callback() {
