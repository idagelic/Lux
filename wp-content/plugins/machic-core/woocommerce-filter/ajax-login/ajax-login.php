...
## Ajax Login Scripts
*************************************************/ 
function machic_ajax_login_scripts() {
	wp_enqueue_script( 'wc-password-strength-meter');
	wp_enqueue_script( 'machic-ajax-login',   plugins_url( 'js/ajax-login.js', __FILE__ ), false, '1.0');
	wp_enqueue_style( 'machic-ajax-login',    plugins_url( 'css/ajax-login.css', __FILE__ ), false, '1.0');
	wp_localize_script( 'machic-ajax-login', 'MyAjax', array(
		'ajaxurl' => esc_url(admin_url( 'admin-ajax.php' )),
	));
}
add_action( 'woocommerce_before_customer_login_form', 'machic_ajax_login_scripts' );

/*************************************************
## Ajax Login CallBack
*************************************************/ 
add_action( 'wp_ajax_nopriv_ajax_login', 'machic_ajax_login_callback' );
add_action( 'wp_ajax_ajax_login', 'machic_ajax_login_callback' );
function machic_ajax_login_callback() {
	...
	try {
		...
		if ( $validation_error->get_error_code() ) {
			throw new Exception( '<strong>' . esc_html__( 'Error:', 'machic-core' ) . '</strong> ' . $validation_error->get_error_message() );
		}

		if ( empty( $creds['user_login'] ) ) {
			throw new Exception( '<strong>' . esc_html__( 'Error:', 'machic-core' ) . '</strong> ' . esc_html__( 'Username is required.', 'machic-core' ) );
		}
		...
		if ( $e->getMessage() ) {
			echo machic_sanitize_data($e->getMessage());
			exit;
		}
	}
}
...
*************************************************/ 
add_action( 'wp_ajax_nopriv_ajax_register', 'machic_ajax_register_callback' );
add_action( 'wp_ajax_ajax_register', 'machic_ajax_register_callback' );
function machic_ajax_register_callback() {
	...
	try {
		...
		if ( 1 === count( $validation_errors ) ) {
			...
			foreach ( $validation_errors as $message ) {
				wc_add_notice( '<strong>' . esc_html__( 'Error:', 'machic-core' ) . '</strong> ' . $message, 'error' );
			}
			throw new Exception();
		}
		...
		if ( 'yes' === get_option( 'woocommerce_registration_generate_password' ) ) {
			wc_add_notice( esc_html__( 'Your account was created successfully and a password has been sent to your email address.', 'machic-core' ) );
		} else {
			wc_add_notice( esc_html__( 'Your account was created successfully. Your login details have been sent to your email address.', 'machic-core' ) );
		}
