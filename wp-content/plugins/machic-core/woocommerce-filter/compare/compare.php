	...
	class KlbCompare {
		...
		function init() {
			...
			self::$fields = apply_filters( 'klbcp_fields', [
				'remove'             => '&nbsp;',
				'image'             => esc_html__( 'Image', 'machic-core' ),
				'name'             	=> esc_html__( 'Name', 'machic-core' ),
				'sku'               => esc_html__( 'SKU', 'machic-core' ),
				'rating'            => esc_html__( 'Rating', 'machic-core' ),
				'price'             => esc_html__( 'Price', 'machic-core' ),
				'stock'             => esc_html__( 'Stock', 'machic-core' ),
				'weight'            => esc_html__( 'Weight', 'machic-core' ),
				'add_to_cart'       => esc_html__( 'Add to cart', 'machic-core' ),
			] );
			...
			add_shortcode( 'klbcp_list', [ $this, 'machic_compare_list' ] );

			// add button for machic_compare_action - product box
			add_action( 'machic_compare_action', [ $this, 'machic_compare_button' ], 11 );
		...
		function enqueue_scripts() {
			...
			wp_localize_script( 'klbcp-frontend', 'klbcp_vars', [
					...
					'view_page_text'     => esc_html__( 'View Compare Page', 'machic-core' ),
					'close_text'     	 => esc_html__( 'Close', 'machic-core' ),
					'message_added'      => esc_html__( '{name} has been added to Compare list.', 'machic-core' ),
					'message_removed'    => esc_html__( '{name} has been removed from the Compare list.', 'machic-core' ),
					'message_exists'     => esc_html__( '{name} is already in the Compare list.', 'machic-core' ),
					'limit'              => 100,
					'limit_notice'       => esc_html__( 'You can add a maximum of {limit} products to the comparison table.', 'machic-core' ),
					'button_text'        => apply_filters( 'klbcp_button_text', esc_html__( 'Compare', 'machic-core' ) ),
					'button_text_added'  => apply_filters( 'klbcp_button_text_added', esc_html__( 'Compare', 'machic-core' ) ),
				]
			);
		}
				...
				foreach ( $products as $product_id ) {
					...
					foreach ( $saved_fields as $saved_field ) {
						switch ( $saved_field ) {
							case 'remove':
								//remove button
								$products_data[ $product_id ]['remove'] = ' <span class="klbcp-remove" data-id="' . $product_id . '"><i class="klbth-icon-cancel"></i> ' . esc_html__( 'Remove', 'machic-core' ) . '</span>';
				...
				$table = '<div class="klbcp-no-result">
							<div class="cart-empty-page">
								<div class="empty-icon">
									<i class="klbth-icon-repeat-1"></i>
								</div>
								<p class="cart-empty">'.esc_html__( 'The comparison table is empty.', 'machic-core' ).'</p>
								<p class="return-to-shop">
									<a class="button wc-backward'.esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ).'" href="'.esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ).'">
										'.esc_html( apply_filters( 'woocommerce_return_to_shop_text', esc_html__( 'Return to shop', 'machic' ) ) ).'
									</a>
								</p>
							</div>
		...
		function machic_compare_button( $attrs ) {
			...
			if ( $attr_id ) {
				// button text
				$text = esc_html__( 'Compare', 'machic-core' );
		...
		function machic_compare_list( $attrs ) {
		...
		function account_items( $items ) {
			...
			if ( ! isset( $items['compare'] ) ) {
				$items['compare'] = apply_filters( 'klbcp_myaccount_compare_label', esc_html__( 'Compare', 'machic-core' ) );
			}
		...
		public static function get_page_url() {
			$page_id  = get_theme_mod('machic_compare_page');
